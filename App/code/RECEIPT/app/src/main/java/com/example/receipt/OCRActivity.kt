...
class OCRActivity : PermissionActivity() {
    ...
    fun processImage(bitmap : Bitmap){
        ...
        val extractOcr: ArrayList<String> = ocr_extract(ocrResult) as ArrayList<String>
        recycle(extractOcr)
    ...
    fun textExtract(extractAllingre : List<String>, ingredient_list : List<String>): List<String>{
        //extractAllingre = 영수증에서 추출된 하나의 요소들
        //ingredient_list = 미리 지정해놓은 재료 카테고리
        var displaylist = ArrayList<String>()

        for(i in ingredient_list) {
            val displaying = extractAllingre.filter { X -> X.contains(i) }
    ...
    fun ocr_extract(ocr_text : String): List<String> {
        var ocr_list = ocr_text.split("\\n")
        var result_list = textExtract(ocr_list,ingredient_list)
        return result_list
    }

    fun recycle(ingre_result: ArrayList<String>) {
        val today: LocalDate = LocalDate.now()
        val profilelist = ArrayList<Profiles>()
        for (i in ingre_result){
            profilelist.add(Profiles(R.drawable.orange, i,"3일",today))
        }
